...
int Calculate_Lung_Capacity(void)
{
    ...
    { 
        ...
        //计算气压
        Capacity.airPressure =  (((u32)9048*ADCBuf[i])/(PRECISION *PRECISION)) -(u32)1930 ;
        //计算单位时间内气体流量
        Capacity.unitGasFlow = (PITOT * (u32)sqrt(((u32)2*Capacity.airPressure - AIRDENSITY))*PIPESIZE)/(PRECISION *PRECISION);        
        //将计算出来的值乘以时间
        Capacity.unitGasFlow =  (Capacity.unitGasFlow*(u32)5)/((u32)(10*PRECISION));
        //累加气体流量
