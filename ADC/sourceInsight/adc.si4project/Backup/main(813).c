...
int Calculate_Lung_Capacity(void)
{
    ...
    { 
        //计算气压
        Capacity.airPressure =  (9048*ADCBuf[i]);
        //计算气压
        Capacity.airPressure =  (9048*ADCBuf[i]-1930)/(PRECISION) ;
        //计算单位时间内气体流量
        Capacity.unitGasFlow = (PITOT * (u32)sqrt((2*Capacity.airPressure - AIRDENSITY))*PIPESIZE)/(PRECISION *PRECISION);        
        //将计算出来的值乘以时间
        Capacity.unitGasFlow =  (Capacity.unitGasFlow*5)/1000;
        //累加气体流量
